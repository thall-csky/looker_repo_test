...
view: sat_product {
	...
	dimension: part_id {		type: string
		sql: ${TABLE}."PART_ID" ;;
	}
		

	dimension_group: create_date {|		timeframes: [raw, time, date, week, month, quarter, year]		type: time
	...
	dimension: subscription_duration {		type: number
		sql: ${TABLE}."SUBSCRIPTION_DURATION" ;;
	}
		

	dimension: access_code_type {		type: string
		sql: ${TABLE}."ACCESS_CODE_TYPE" ;;
	}
		

	dimension: pubco_publisher_id {		type: string
		sql: ${TABLE}."PUBCO_PUBLISHER_ID" ;;
	}
		

	dimension: pubco_product_id {		type: string
