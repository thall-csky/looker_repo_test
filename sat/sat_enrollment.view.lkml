...
view: sat_enrollment {
	...
	dimension: access_role {		type: string
		sql: ${TABLE}."ACCESS_ROLE" ;;
	}
		

	dimension: self_study_isbn13 {		type: string
		sql: ${TABLE}."SELF_STUDY_ISBN13" ;;
	}
		

	dimension_group: enrollment_date {|		timeframes: [raw, time, date, week, month, quarter, year]		type: time
		sql: ${TABLE}."ENROLLMENT_DATE" ;;
	}
		

	dimension: paid_in_full {		type: string
		sql: ${TABLE}."PAID_IN_FULL" ;;
	}
		

	dimension: order_number {		type: string
		sql: ${TABLE}."ORDER_NUMBER" ;;
	}
		

	dimension: payment_code {		type: string
		sql: ${TABLE}."PAYMENT_CODE" ;;
	}
		

	dimension_group: grace_end_date {|		timeframes: [raw, time, date, week, month, quarter, year]		type: time
		sql: ${TABLE}."GRACE_END_DATE" ;;
	}
		

	dimension: transferred {		type: string
		sql: ${TABLE}."TRANSFERRED" ;;
	}
		

	dimension: opt_out_institution_purchase {		type: string
	...
	dimension: access_code_swap {		type: string
		sql: ${TABLE}."ACCESS_CODE_SWAP" ;;
	}
		

	dimension: access_code_isbn13 {		type: string
		sql: ${TABLE}."ACCESS_CODE_ISBN13" ;;
	}
		

	dimension: access_code_price {		type: number
		sql: ${TABLE}."ACCESS_CODE_PRICE" ;;
	}
		

	dimension: course_isbn_price {		type: number
	...
	dimension: payment_isbn13 {		type: string
