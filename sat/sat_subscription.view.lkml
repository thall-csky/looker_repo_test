...
view: sat_subscription {
	...
	dimension: link_user_contract_key {|		primary_key: yes		type: string
	...
	dimension: subscription_state {		type: string
		sql: ${TABLE}."SUBSCRIPTION_STATE" ;;
	}
		

	dimension_group: start_timestamp {|		timeframes: [raw, time, date, week, month, quarter, year]		type: time
		sql: ${TABLE}."START_TIMESTAMP" ;;
	}
		

	dimension_group: end_timestamp {|		timeframes: [raw, time, date, week, month, quarter, year]		type: time
	...
	dimension: subscription_id {		type: string
		sql: ${TABLE}."SUBSCRIPTION_ID" ;;
	}
		

	dimension_group: available_until {|		timeframes: [raw, time, date, week, month, quarter, year]		type: time
		sql: ${TABLE}."AVAILABLE_UNTIL" ;;
	}
		

	dimension: plan_id {		type: string
		sql: ${TABLE}."PLAN_ID" ;;
	}
		

	dimension: duration {		type: string
		sql: ${TABLE}."DURATION" ;;
	}
		

	dimension_group: placed_time {|		timeframes: [raw, time, date, week, month, quarter, year]		type: time
		sql: ${TABLE}."PLACED_TIME" ;;
	}
		

	dimension_group: cancelled_time {|		timeframes: [raw, time, date, week, month, quarter, year]		type: time
		sql: ${TABLE}."CANCELLED_TIME" ;;
	}
		

	dimension: cancellation_reason {		type: string
		sql: ${TABLE}."CANCELLATION_REASON" ;;
	}
		

	dimension: payment_source_type {		type: string
		sql: ${TABLE}."PAYMENT_SOURCE_TYPE" ;;
	}
		

	dimension: payment_source_id {		type: string
		sql: ${TABLE}."PAYMENT_SOURCE_ID" ;;
	}
		

	dimension: payment_source_guid {		type: string
		sql: ${TABLE}."PAYMENT_SOURCE_GUID" ;;
	}
		

	dimension: payment_source_line {		type: string
